...
// Here is a list of things that definitly _DONT_ work correctly.
...
reflect_parse_glsl_src_code :: proc (src_code : []u8) -> ReflectInfo {
	...
	for line in strings.split_lines_iterator(&src_code_str) {
		...
		if strings.contains(line_str, "local_size_x"){
			...
			no_tabs  , _ := strings.remove_all(line_str, "\t", context.temp_allocator)
			no_spaces, _ := strings.remove_all(no_tabs , " ", context.temp_allocator)
